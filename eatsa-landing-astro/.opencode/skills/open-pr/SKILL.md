...
| `production` (default) | Full `skill({ name: "check-pr-readiness" })` against the branch vs. base. Blocks on any red gate (typecheck, lint, formatter, tests, residue, large/binary additions, lockfile drift). |
...
Stop conditions:
- Uncommitted changes → use `question` tool to ask whether to commit, stash, or include them. Do not silently ignore.
...
2. If there are unpushed commits, synthesize a default branch name from the cumulative commits/diff (e.g., `feat/<short-subject>`, `fix/<short-subject>` — kebab-case, ≤40 chars). Use `question` tool to ask:
...
- `mode=production` → invoke `skill({ name: "check-pr-readiness" })` and pipe its report through verbatim.
- `mode=balanced` / `mode=fast` → run the scoped checks listed in the table.

If any gate fails, **stop**. Use `question` tool to ask:
...
- **NEVER open a PR against a protected branch you can't target**
  **Instead:** If `gh pr create` fails with a permission/protection error, surface it; use `question` tool to ask whether to retarget or open from a fork.
