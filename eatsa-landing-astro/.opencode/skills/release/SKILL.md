...
| `production` (default) | Full `check-pr-readiness` (loaded via `skill({ name: "check-pr-readiness" })`) against the release range (last tag → HEAD, or all history when no prior tag). Blocks on any red gate. |
...
1. **Dirty tree** (`git status --porcelain` non-empty) → STOP. Show the user the dirty files and ask whether to stash, commit separately, or abort via `question` tool. Do not auto-stash.
2. **Branch check** — if not on `main` / `master` / `release/*`, ask the user to confirm before continuing via `question` tool. Releasing from a feature branch is almost always a mistake.
...
4. **Detect manifest** (first match wins, in this order):
   ...
   - None found → STOP and ask the user where the version lives via `question` tool.
...
- `mode=production` → invoke `check-pr-readiness` by loading `skill({ name: "check-pr-readiness" })` against the release range. Pipe its report through verbatim.
...
If any gate fails, **stop**. Ask via `question` tool with options:
...
Show the notes to the user. Ask via `question` tool with options: `(a)ccept / (e)dit / (q)uit`. On `e`, open the user's editor or accept inline edits.
...
Only run `git push --follow-tags` on explicit `y` via `question` tool. Default is no-push so the user can review or amend.
...
- **NEVER push the tag without explicit user confirmation in this turn**
  **Instead:** Stop after creating the local tag and ask via `question` tool. A prior session's "yes" doesn't carry forward.
