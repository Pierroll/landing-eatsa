...
Detect the analytics SDK by imports. Use `codegraph_search` to find analytics-related symbols. If `.codegraph/` not initialized, fall back to `grep`.
...
For the user's scope (one route / feature / module), enumerate user-facing actions and check each. Use `codegraph_explore` with relevant route/component names. If `.codegraph/` not initialized, fall back to `read` + `grep`.
