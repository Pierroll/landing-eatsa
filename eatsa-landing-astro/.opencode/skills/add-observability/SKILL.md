...
- **Sweep** — invoked after a feature ships (often after feature addition). Scope = `git diff <base>..HEAD` filtered to source files.

If mode is ambiguous, ask once via `question` tool:

```
question({
  questions: [{
    header: "Scope mode",
    question: "Targeted (which file/feature?) or sweep over the current branch's diff?",
    options: [
      { label: "Targeted", description: "Name a specific file, route, or integration" },
      { label: "Sweep", description: "Sweep git diff of current branch" }
    ]
  }]
})
```
...
1. **Imported logger** — use `codegraph_search` or grep scoped files and shared `lib/`/`src/utils/` for `from ['"]pino`, `from ['"]winston`, `from ['"]electron-log`, `import debug from ['"]debug`, `from ['"]bunyan`, `from ['"]@logtape`, `loglevel`. If `.codegraph/` not initialized, fall back to `grep`.
...
**Before installing anything, ask the user.** Frame via `question` tool:

```
question({
  questions: [{
    header: "No logger detected",
    question: "Choose logging approach:",
    options: [
      { label: "Zero-dep console fallback", description: "console-based logger with LOG_LEVEL env gate (no install)" },
      { label: "Install pino", description: "Tiny, fast, structured (npm install required)" },
      { label: "Point to existing logger", description: "I'll tell you which logger I missed" }
    ]
  }]
})
```
...
For each in-scope file, list every boundary call site as `path:line — <type>`. Use `codegraph_explore` first to identify the structure. If `.codegraph/` not initialized, fall back to `read` + `grep`.
...
- **hook** — hook scripts, framework lifecycle hooks invoked by external systems
- **mcp** — MCP server tool handlers
- **silent-failure** — `|| true`, empty `catch {}`, `>/dev/null 2>&1`, `.catch(() => {})` on the dispatch path

Show this list to the user and confirm before instrumenting. **Exit condition:** confirmed list. If >20 boundaries, group by file and ask via `question` tool which to prioritize — do not bulk-edit a sprawling list without acknowledgment.
...
- **NEVER install a logger dependency without asking**
  **Instead:** Offer the zero-dep fallback first via `question` tool; install only on explicit user approval.
