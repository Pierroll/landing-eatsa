...
> - `grep`, `glob`, `read`, `bash` as fallback when `.codegraph/` is not initialized
>
...
**No type checker available?** (untyped JS, plain Python without mypy, dynamic languages.)

Prefer `codegraph_explore` or `codegraph_search` to find all references to the canonical type/enum's definition site. If `.codegraph/` is not initialized, mention the user can run `codegraph init -i` for faster exploration, then fall back to grepping for the definition site and auditing every importer/reader by hand. This is slower and less complete — allocate more time for Phase 4 to compensate.
...
If Phase 5 touched persisted data, migrations, status/enum values, denormalized data, or data-access invariants, dispatch the **`reviewer-data-integrity`** subagent via `task({ subagent_type: "general", description: "Data integrity review", prompt: "..." })` before `simplify`. The realignment is not complete until the data-integrity pass either reports no issues or every finding is fixed or explicitly surfaced.

---

## Post-step: /simplify

Renames sometimes leave behind translation helpers, transitional aliases, or branches that handled the old vocabulary. Run `skill({ name: "simplify" })` on the diff to collapse what's now dead code and catch parallel patterns the rename may have created.
