...
3. **Resolve per block:**
   ...
   | Incompatible (both edit the same logic differently) | **Stop.** Surface to the user via `question` tool with both versions side-by-side. Do not guess. |
...
- **NEVER guess on incompatible blocks.**
  **Instead:** stop, present both versions, use `question` tool to ask the user which intent should win.
