...
Prefer `codegraph_explore` to map the module's public surface (symbols, signatures, dependencies) in one shot. If `.codegraph/` is not initialized, fall back to `read` + `grep` for the surface scan.
...
Ask via the `question` tool: `Approve install? (y/n)`.
