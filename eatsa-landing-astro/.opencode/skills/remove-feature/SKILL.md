...
Prefer `codegraph_explore`, `codegraph_search`, or `codegraph_callers` for symbol-level reference mapping. If `.codegraph/` is not initialized, mention the user can run `codegraph init -i` for faster exploration, then fall back to `grep` + `read` + `glob`. **Note:** string-based references (route paths, i18n keys, feature flags, CSS classes, config values) still require grep — codegraph cannot find these.
...
- `skill({ name: "add-migration" })` — when the removal changes schema, drops/stops-writing columns, removes enum values from persisted rows, changes indexes/constraints, or requires a backfill/purge/historical-data decision.
- **`task({ subagent_type: "reviewer-data-integrity", description: "Review data integrity after removal", prompt: "Review the removal diff for data integrity issues. Focus on persistence shape changes, migrations, status/enum values, denormalized data, deletes, backfills, import/export paths, and data-access invariants. Return severity-ranked findings." })`** — after any persisted-data, migration, delete cascade, import/export, seed-data, or data-access-invariant change.
- **`task({ subagent_type: "reviewer-contracts", description: "Review contract compatibility after removal", prompt: "Review the removal diff for contract breaks across public URLs, route params, API responses, DTOs, server functions, OpenAPI/tRPC/generated clients, package exports, webhook payloads, and generated schemas. Return severity-ranked findings." })`** — when the removal touches public URLs, route params, API responses, DTOs, server functions, OpenAPI/tRPC/generated clients, package exports, webhook payloads, or generated schemas.
- **`task({ subagent_type: "reviewer-error-boundaries", description: "Review error boundaries after removal", prompt: "Review the removal diff for failure UX gaps in adjacent user flows. Focus on changed failure paths, fallback routes, empty states, and loader errors caused by the removal. Return findings." })`** — when the removed feature leaves adjacent user flows with changed failure paths, fallback routes, empty states, or loader errors.
- **`task({ subagent_type: "reviewer-client-bundle", description: "Review client bundle after removal", prompt: "Review the removal diff for client bundle regressions. Focus on stale dynamic imports, unused large dependencies, and server/client import drift after routes/components/dependencies are removed. Return findings." })`** — when the removal deletes or moves client routes/components/dependencies and may leave stale dynamic imports, unused large deps, or server/client import drift.
- `skill({ name: "sync-docs" })` — recommend or invoke when existing docs, API docs, README, `.env.example`, setup docs, or runbooks mention the removed feature. This skill updates existing docs only; it does not create new files.
- `skill({ name: "update-changelog" })` — invoke when the removal is user-visible or engineering-significant and the project has a changelog convention.
...
After removal, surrounding code often loses a reason to exist as a separate abstraction (lone-caller helpers, single-branch conditionals, parameters only used by the deleted path). Run `skill({ name: "simplify" })` against the diff to collapse the leftovers.
