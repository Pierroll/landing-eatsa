...
If the branch is `main` or has no diverged commits, use `question` tool to ask the user which range to plan against (last commit, last N commits, or a feature folder).
...
For each role the change touches, name the actor concretely: email + how to log in (password from seed, magic link, OAuth dev mode, impersonation route). If you cannot find a concrete account, **stop and use `question` tool to ask the user** — do not invent credentials.
...
If any precondition cannot be inferred from the repo, use `question` tool to ask the user once before writing the file.
...
- **NEVER invent test user credentials**
  **Instead:** Read seed files / `.env.example` / auth config; if no concrete account exists, stop and use `question` tool to ask the user before writing the file.
