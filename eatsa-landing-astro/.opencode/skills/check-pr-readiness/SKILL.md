...
If `format` failed and a `format` (write) script exists (`format`, `prettier:write`, `lint:fix` for stylistic-only rules), offer to run it via `question` tool:
...
- **NEVER auto-fix lint or test failures.**
  **Instead:** report the failure with the exact command to reproduce. Auto-fix only format issues, and only after user `y` via `question` tool.
