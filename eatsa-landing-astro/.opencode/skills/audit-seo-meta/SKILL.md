...
Use `codegraph_files` to list routes. If `.codegraph/` not initialized, mention user can run `codegraph init -i`, then fall back to `glob` + `grep`.

If the user can't tell which routes are public, ask via `question` tool before scanning the whole repo.
...
Use the `question` tool to ask for user approval before applying any auto-fix.
...
- **NEVER auto-fix `robots.txt` to include `Disallow: /` or `Allow: /` blanket rules without confirming with the user.**
  **Instead:** use the `question` tool to propose the file content, show it, and require approval.
