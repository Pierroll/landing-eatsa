...
Default scope = a route or component the user names. The skill follows imports one layer deep (the route file plus its directly-imported feature components, not the whole UI library). Use `codegraph_explore` to trace imports. If `.codegraph/` not initialized, mention user can run `codegraph init -i`, then fall back to `grep` + `read` + `glob`.
