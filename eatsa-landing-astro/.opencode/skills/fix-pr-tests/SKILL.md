...
4. **Ambiguous?** When you cannot tell whether the new behavior is intentional, **stop and ask the user**. Do not guess. Call the `question` tool to present: "Test `X` expects `A`, code now returns `B`. Is `B` the intended new behavior (I'll update the test) or a regression (I'll fix the code)?"
...
After CI is green, run `skill({ name: "simplify" })` on the fix diff — test-fix patches often introduce duplicated helpers, magic numbers, or shortcut code that should be cleaned up before merging.
