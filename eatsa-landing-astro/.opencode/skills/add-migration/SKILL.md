...
If none detected and the user wants a migration, ask via `question` tool which they use rather than picking one.
...
For dropping rows (data deletion), get an explicit user confirmation via `question` tool and require a backup pointer (snapshot SHA, dump filename) be recorded in the migration's accompanying notes file.
...
After the migration file and any backfill script are in place, dispatch a data-integrity reviewer subagent against the migration + related code diff:

```
task({
  subagent_type: "general",
  description: "Review migration data integrity",
  prompt: "Audit the following migration for data-integrity: check defaults/backfills, destructive rollout order, uniqueness assumptions, delete cascades, seed/test fixture drift, and data-access invariants. Migration: <path>. Related code diff: <describe>."
})
```
...
- **NEVER drop or truncate data without an explicit user `y` and a recorded backup pointer.**
  **Instead:** confirm via `question` tool; record the pre-migration snapshot SHA, dump file path, or PITR timestamp in a notes file alongside the migration.
