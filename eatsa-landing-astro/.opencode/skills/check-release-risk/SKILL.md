...
If the branch has user-facing changes (categorizer 1, 3, or 5 hits) but no doc/changelog edits: **MEDIUM** — recommend running the `update-changelog` skill (via `skill({ name: "update-changelog" })`) and doc-sync before publishing.
...
- **No CHANGELOG entry** for user-visible changes — consider running `skill({ name: "update-changelog" })`.
...
This is informational. Proceed when ready:
  • commit-and-push (via `skill({ name: "commit-and-push" })`)
  • open-pr
  • release (via `skill({ name: "release" })`)
```
...
- **NEVER auto-fix issues identified in the briefing**
  **Instead:** Recommend the relevant skill (doc-sync, `skill({ name: "update-changelog" })`, or the reviewer-data-integrity subagent) and let the user run it.
...
- **NEVER assume the base branch is `main`**
  **Instead:** Resolve `origin/HEAD` first; fall back to `main` → `master` → `develop`. If still unresolved, ask once via `question` tool.
