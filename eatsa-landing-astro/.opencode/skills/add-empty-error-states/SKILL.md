...
Prefer `codegraph_explore` with symbol names to trace the data flow; fall back to `grep` + `read` if `.codegraph/` not initialized. If `.codegraph/` not initialized, mention the user can run `codegraph init -i`.
...
Search the codebase for an existing empty/error component. Prefer `codegraph_search` with query "EmptyState" or "ErrorState"; fall back to `grep` + `glob` if `.codegraph/` not initialized:
...
If a primitive exists, use it. If not, propose the smallest one that matches the project's component style — but flag this as a structural decision and let the user confirm via `question` tool before creating shared components.
