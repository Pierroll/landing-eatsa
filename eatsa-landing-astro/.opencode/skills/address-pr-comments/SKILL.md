...
For each unresolved thread, in order from oldest to newest, **dispatch a resolver subagent** (via `task({ subagent_type: "general", description: "Resolve PR comment thread", prompt: "..." })`) — one subagent invocation per thread, sequentially. Each subagent isolates the read-edit-commit-reply-resolve work in a fresh context, so the parent's window stays clean as you work through 10+ threads.
...
### 3a. Ground the fix
- Read the file at the comment's `path` and `line`. Prefer `codegraph_explore` with the symbol/file name for context; fall back to `grep` + `read` if `.codegraph/` not initialized.
...
After all comment commits land, run `skill({ name: "simplify" })` against the cumulative diff to catch duplication or smell drift introduced across the per-comment fixes.
