...
Use the `question` tool to ask. Wait for `y`. Do not auto-install. If the user declines, stop.
...
- **NEVER write an e2e test for logic that has no UI surface.**
  **Instead:** redirect to `skill({ name: "write-tests" })` for an integration test that hits the function/API directly.
