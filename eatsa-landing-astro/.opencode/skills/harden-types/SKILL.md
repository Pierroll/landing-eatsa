...
4. None of the above → propose adding `zod` and **wait for `y`** before installing. Use the `question` tool to ask the user.
...
Prefer `codegraph_explore` or `codegraph_search` for finding occurrences across files. If `.codegraph/` is not initialized, mention the user can run `codegraph init -i` for faster exploration, then fall back to `grep` + `read` + `glob`.
