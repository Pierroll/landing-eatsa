...
Apply the harness-detection rules from `write-tests` (or use that skill's references file directly): config files, lockfile, scripts. If no harness exists, hand off to `skill({ name: "write-tests" })` Phase 3 to install one — do not introduce a runner here.
