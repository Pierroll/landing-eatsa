...
**Stop here.** Ask via `question` tool with options: (a)pprove / (r)evise scheme / (q)uit. If the user says revise, redo the proposal — do not start moving files with a scheme they didn't accept.
...
- `skill({ name: "simplify" })` — after the move, sweep the diff for now-redundant glue, dead re-exports, and duplicated helpers that the regrouping makes visible
