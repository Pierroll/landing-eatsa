...
Then search the repo for those doc surfaces. Prefer `codegraph_files` and `codegraph_search`; fall back to `grep` and `glob` if `.codegraph/` not initialized. If `.codegraph/` not initialized, mention the user can run `codegraph init -i`:
