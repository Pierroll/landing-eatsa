...
import 'kleur/colors';
import { $ as $$AdminLayout } from '../../chunks/AdminLayout_BwPl1C7R.mjs';
import { g as getProductos } from '../../chunks/db_DJSxRCfF.mjs';
...
+ Nuevo Producto
</button> </div> <div class="bg-white rounded-xl shadow-sm border border-gris-claro overflow-hidden"> <table class="w-full text-left border-collapse"> <thead> <tr class="bg-crema border-b border-gris-claro text-sm text-gris-medio"> <th class="p-4 font-medium">Imagen</th> <th class="p-4 font-medium">ID / Slug</th> <th class="p-4 font-medium">Nombre</th> <th class="p-4 font-medium">Estado</th> <th class="p-4 font-medium text-right">Acciones</th> </tr> </thead> <tbody> ${productos.length === 0 ? renderTemplate`<tr> <td colspan="5" class="p-8 text-center text-gris-medio">No hay productos registrados.</td> </tr>` : productos.map((p) => renderTemplate`<tr class="border-b border-gris-claro last:border-0 hover:bg-gray-50 transition-colors"> <td class="p-4"> <img${addAttribute(p.image, "src")}${addAttribute(p.name, "alt")} class="w-16 h-16 object-cover rounded-md border border-gris-claro"> </td> <td class="p-4 text-sm text-gris-medio font-mono">${p.id}</td> <td class="p-4 font-medium text-ebano">${p.name}</td> <td class="p-4"> <span${addAttribute(`inline-block px-2 py-1 text-xs rounded-full ${p.status === "activo" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`, "class")}> ${p.status} </span> </td> <td class="p-4 text-right"> <button class="text-sm text-verde hover:text-cacao transition-colors font-medium edit-btn"${addAttribute(JSON.stringify(p), "data-product")}>Editar</button> </td> </tr>`)} </tbody> </table> </div>  <div id="product-modal" class="fixed inset-0 bg-ebano/50 z-50 hidden flex items-center justify-center p-4"> <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6"> <h2 id="modal-title" class="text-xl font-bold text-verde mb-4">Nuevo Producto</h2> <form id="product-form" class="flex flex-col gap-4"> <div> <label class="block text-sm font-medium text-ebano mb-1">ID / Slug (ej. cacao-fino)</label> <input type="text" name="id" required class="w-full border border-gris-claro rounded-btn p-2"> </div> <div> <label class="block text-sm font-medium text-ebano mb-1">Nombre Público</label> <input type="text" name="name" required class="w-full border border-gris-claro rounded-btn p-2"> </div> <div> <label class="block text-sm font-medium text-ebano mb-1">Ruta de la Imagen (temporalmente URL /ruta.jpg)</label> <input type="text" name="image" value="/assets/company/cacao-fino.jpg" required class="w-full border border-gris-claro rounded-btn p-2"> </div> <div> <label class="block text-sm font-medium text-ebano mb-1">Estado</label> <select name="status" class="w-full border border-gris-claro rounded-btn p-2"> <option value="activo">Activo</option> <option value="inactivo">Inactivo</option> </select> </div> <div class="flex justify-end gap-2 mt-4"> <button type="button" id="close-modal-btn" class="btn-secondary">Cancelar</button> <button type="submit" class="btn-primary">Guardar</button> </div> </form> </div> </div>  ` })}`;
